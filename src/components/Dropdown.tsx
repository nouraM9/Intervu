...
function Dropdown({ label, options, value, onChange }: DropdownProps) {
  ...
  return (
    <div className="relative mb-6">
      ...
      {open && (
        <div className="absolute w-full bg-white rounded-xl shadow-xl z-10">
          {options.map((option) => (
            <button
              className="block w-full text-left body-text-1 px-4 py-3 hover:bg-(--accent) hover:text-white "
              key={option}
              type="button"
              value={option}
              onClick={() => {
